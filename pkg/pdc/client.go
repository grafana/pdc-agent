...
type Config struct {
	...
	// Port for making connections to the PDC API.
	// This is useful for when outbound requests from the PDC agent need to be forwarded through some other networking infrastructure in a customer network.
	Port int
...
func (cfg *Config) RegisterFlags(fs *flag.FlagSet) {
	...
	fs.IntVar(&cfg.Port, "api-port", 443, "The port to use for agent connections to the PDC API. Useful for if the pdc agent connections are getting forwarded through some proxies in your network.")
